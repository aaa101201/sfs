    ...
    */
    ...
    var _Private = {
        ...
        importClippingtoImage: function () {
            ...
            var imageWidth = vimg.width * vimg.scale;
            //根据图片实际大小计算放大倍数
            // var magnify = Math.max(vimg.naturalWidth / imageWidth, 1); //annotation by zt at 2017/06/19
            var magnify = vimg.naturalWidth / imageWidth; //write by zt at 2017/06/19
            //绘制层
            ...
            //判断是否需要处理ios拍照上传问题,by zoutong at 2017/06/21
            if(s.Orientation != "" && s.Orientation != 1){
                outputContext.save();
                outputContext.translate(outputCanvas.width / 2, outputCanvas.height / 2);
                switch(s.Orientation){
                    case 6://需要顺时针（向左）90度旋转
                        outputContext.rotate(90*Math.PI/180);
                        break;
                    case 8://需要逆时针（向右）90度旋转
                        outputContext.rotate(-90*Math.PI/180);
                        break;
                    case 3://需要180度旋转
                        outputContext.rotate(180*Math.PI/180);
                        break;
                }
            ...
            s.Orientation = null;
            return outputCanvas.toDataURL('image/' + s.clippingImportSuffix, 1);
        },
        //判断事件目标是否在图像列表中
